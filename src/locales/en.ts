export default {
  common: {
    send: 'Send',
    clear: 'Clear',
    cancel: 'Cancel',
    confirm: 'OK',
    refresh: 'Refresh',
    settings: 'Settings',
    open: 'Open',
    close: 'Close',
    connected: 'Connected',
    notConnected: 'Disconnected',
    bytes: 'bytes',
    delete: 'Delete',
  },
  app: {
    serialPort: 'Serial Port',
    modbus: 'Modbus',
    mqtt: 'MQTT',
    opcua: 'OPC UA',
    iec104: 'IEC104',
    newVersionTitle: 'New Version Available',
    newVersionMsg: 'Latest version: {version}. Please visit GitHub to download the update.',
    upToDate: 'You are on the latest version',
    updateCheckFailed: 'Update check failed: ',
  },
  serial: {
    port: 'Port:',
    baudRate: 'Baud Rate:',
    selectPort: 'Please select a serial port!',
    selectBaudRate: 'Please select a baud rate!',
    opened: 'Serial port opened',
    openFailed: 'Failed to open port: ',
    inputSendContent: 'Please enter send content',
    sendFailed: 'Send failed: ',
    sendSettings: 'Send Settings',
    pollingSend: 'Polling Send',
    sendContent: 'Send Content',
    receiveSettings: 'Receive Settings',
    receiveContent: 'Receive Content',
    receive: 'RX: ',
    sendBytes: 'TX: ',
    portSettings: 'Serial Port Settings',
    dataBits: 'Data Bits',
    stopBits: 'Stop Bits',
    parity: 'Parity',
    flowControl: 'Flow Control',
  },
  mqtt: {
    host: 'Host:',
    port: 'Port:',
    clientId: 'Client ID:',
    username: 'Username:',
    password: 'Password:',
    optional: 'optional',
    protocol: 'Protocol:',
    connect: 'Connect',
    disconnect: 'Disconnect',
    subscribe: 'Subscribe',
    unsubscribe: 'Unsubscribe',
    subscribedTopics: 'Subscribed Topics',
    subscribed: 'Subscribed',
    unsubscribed: 'Unsubscribed',
    noSubscriptions: 'No subscriptions',
    publish: 'Publish',
    topic: 'Topic:',
    payload: 'Payload:',
    qos: 'QoS:',
    retain: 'Retain',
    commLog: 'Message Log',
    connectSuccess: 'Connected successfully',
    connectFailed: 'Connection failed: ',
    disconnectedMsg: 'Disconnected',
    connLost: 'Connection lost',
    subscribeFailed: 'Subscribe failed: ',
    publishFailed: 'Publish failed: ',
    inputTopic: 'Enter topic',
    inputPayload: 'Enter payload',
    inputHost: 'Enter host address',
    connectedTo: 'Connected to {host}:{port}',
  },
  opcua: {
    endpoint: 'Endpoint:',
    username: 'Username:',
    password: 'Password:',
    optional: 'optional',
    connect: 'Connect',
    disconnect: 'Disconnect',
    nodeIdPlaceholder: 'e.g. ns=2;i=1001',
    addRead: 'Add & Read',
    timeout: 'Timeout:',
    nodeId: 'Node ID',
    value: 'Value',
    dataType: 'Data Type',
    status: 'Status',
    sourceTimestamp: 'Source Timestamp',
    localTime: 'Local Time',
    actions: 'Actions',
    refresh: 'Refresh',
    edit: 'Edit',
    write: 'Write',
    noNodes: 'No nodes added yet',
    connectSuccess: 'Connected successfully',
    connectFailed: 'Connection failed: ',
    disconnectedMsg: 'Disconnected',
    readFailed: 'Read failed: ',
    writeSuccess: 'Write successful',
    writeFailed: 'Write failed: ',
    inputEndpoint: 'Please enter endpoint URL',
    inputNodeId: 'Please enter a node ID',
  },
  modbus: {
    host: 'Host:',
    port: 'Port:',
    unitId: 'Unit ID:',
    connect: 'Connect',
    disconnect: 'Disconnect',
    functionCode: 'Function Code:',
    startAddress: 'Start Address:',
    quantity: 'Quantity:',
    displayFormat: 'Display Format:',
    stopPolling: 'Stop Polling',
    startPolling: 'Live Polling',
    polling: 'Polling...',
    writeData: 'Write Data:',
    address: 'Address',
    hex: 'Hex',
    binary16: 'Binary (16-bit)',
    commLog: 'Communication Log',
    connectFirst: 'Please connect first',
    inputHost: 'Please enter a host address',
    connectSuccess: 'Connected successfully',
    connectFailed: 'Connection failed: ',
    disconnectedMsg: 'Disconnected',
    connLost: 'Connection lost',
    sendFailed: 'Send failed: ',
    connectedTo: 'Connected to {host}:{port}',
    fc: {
      readCoils: 'FC01 - Read Coils',
      readDiscreteInputs: 'FC02 - Read Discrete Inputs',
      readHoldingRegisters: 'FC03 - Read Holding Registers',
      readInputRegisters: 'FC04 - Read Input Registers',
      writeSingleCoil: 'FC05 - Write Single Coil',
      writeSingleRegister: 'FC06 - Write Single Register',
    },
    exception: {
      illegalFunction: 'Illegal Function',
      illegalAddress: 'Illegal Data Address',
      illegalValue: 'Illegal Data Value',
      deviceFailure: 'Slave Device Failure',
      acknowledge: 'Acknowledge',
      deviceBusy: 'Slave Device Busy',
      memoryError: 'Memory Parity Error',
      gatewayPath: 'Gateway Path Unavailable',
      gatewayTarget: 'Gateway Target Device Failed to Respond',
      unknown: 'Unknown Exception',
    },
  },
  iec104: {
    host: 'Host:',
    port: 'Port:',
    commonAddress: 'Common Addr:',
    timeout: 'Timeout:',
    autoStart: 'Start data transfer after connecting',
    connect: 'Connect',
    disconnect: 'Disconnect',
    startDataTransfer: 'STARTDT',
    stopDataTransfer: 'STOPDT',
    testFrame: 'TESTFR',
    generalInterrogation: 'General Interrogation',
    clockSync: 'Clock Sync',
    clockSyncConfirmTitle: 'Confirm Clock Sync',
    clockSyncConfirmMessage: 'Synchronize local time {time} to common address {address}?',
    started: 'Data transfer started',
    rawApdu: 'Raw APDU:',
    objects: 'Information Objects',
    localTime: 'Local Time',
    type: 'Type',
    value: 'Value',
    quality: 'Quality',
    sourceTime: 'Source Time',
    commLog: 'Communication Log',
    inputHost: 'Please enter a host address',
    inputRaw: 'Please enter a hexadecimal APDU',
    connectSuccess: 'Connected successfully',
    connectFailed: 'Connection failed: ',
    sendFailed: 'Send failed: ',
    disconnectedMsg: 'Disconnected',
    connLost: 'Connection lost',
    connectedTo: 'Connected to {host}:{port}',
  },
}
